	...
	go func(errChan chan<- error, progress *mpb.Progress) {
		...
		for dr := range downloads {
			if dr.Err != nil {
				...
				log.Printf("Removing failed download %s...\n", dr.Dependency.DownloadPath)
				err = os.RemoveAll(dr.Dependency.DownloadPath)
				// If there was an error removing the failed download, return
				if err != nil {
					msg := fmt.Sprintf("Error occurred while removing failed download %s", dr.Dependency.DownloadPath)
